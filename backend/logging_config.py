...
Same shape as agent-runner/logging_config.py, duplicated rather than shared
-- the two are independent services with their own requirements.txt/Dockerfile,
matching how settings.py is already duplicated per component.
...
# file lives in <repo>/backend, so logs land at <repo>/logs.
...
def get_logger(name: str, component: str = COMPONENT) -> logging.Logger:
    ...
    module; `component` defaults to "backend" but routers/logs.py overrides
    it to "frontend" when relaying client-side error reports."""
