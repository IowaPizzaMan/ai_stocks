...
def get_stock_news(ticker: str, db: Database | None = None) -> dict:
    ...
    stale = False
    try:
        raw = _fetch_window(ticker, cutoff, today, db)
        db[STOCK_NEWS_CACHE].replace_one(
            {"ticker": ticker},
            {"ticker": ticker, "articles": raw, "fetched_at": datetime.now(timezone.utc)},
            upsert=True,
        )
    except FmpBudgetExceededError:
        logger.warning("%s: FMP budget spent — serving cached news", ticker)
        raw, stale = _cached(ticker, db), True
